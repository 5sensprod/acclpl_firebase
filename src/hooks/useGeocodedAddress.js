...
function addressReducer(state, action) {
  switch (action.type) {
    ...
    case ACTIONS.VALIDATE_ADDRESS:
      return { ...state, isAddressValidated: true }
    case ACTIONS.MODIFY_ADDRESS:
      return { ...state, isAddressValidated: false }
    default:
      return state
  }
}

function useCompanyAddress(onSelectAddress, currentCoords, moveToNextStep) {
  const [state, dispatch] = useReducer(addressReducer, initialState)

  if (!onSelectAddress) return null
  ...
  const handleAddressValidation = () => {
    dispatch({ type: ACTIONS.VALIDATE_ADDRESS })
    if (moveToNextStep) moveToNextStep()
  }

  const handleAddressModification = () => {
    dispatch({ type: ACTIONS.MODIFY_ADDRESS })
  }
  ...
  const handleGeolocationClick = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        onSelectAddress(currentCoords || [lon, lat])
      },
      (error) => {
        switch (error.code) {
  ...
  return {
    ...
    isAddressValidated: state.isAddressValidated,
